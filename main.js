...
/* ── Hero Slideshow — full-bleed crossfade with Ken Burns ── */
(function initHeroSlideshow() {
  ...
  const SLIDES = [
    { img: 'https://www.namiart-gallery.com/cdn/shop/files/RL_LandscapeComposition3.jpg?v=1719800000',
      hebLabel: 'Featured Work', hebTitle: 'Landscape Composition III — Raymond Loyola', href: '/works' },
    { img: 'https://www.namiart-gallery.com/cdn/shop/collections/Summer_Pasana.png?v=1705711338',
      hebLabel: 'Artist Spotlight', hebTitle: 'Summer Pasana', href: '/artists' },
    { img: 'https://www.namiart-gallery.com/cdn/shop/collections/632328236_1359537512523591_3542641750037734077_n.jpg?v=1771207428',
      hebLabel: 'Now Showing', hebTitle: 'Spaces and Stories — Group Exhibition', href: '/exhibitions' },
    { img: 'https://www.namiart-gallery.com/cdn/shop/files/SP_StudyBlueGold.jpg?v=1705711338',
      hebLabel: 'Featured Work', hebTitle: 'Study in Blue and Gold — Summer Pasana', href: '/works' },
    { img: 'https://www.namiart-gallery.com/cdn/shop/collections/Verses_Poster_1080x1350_fb8c78a2-a8cb-435d-ac41-6c112e2f1504.png?v=1769234583',
      hebLabel: 'Past Exhibition', hebTitle: 'Verses — Jonet Carpio', href: '/exhibitions' },
    { img: 'https://www.namiart-gallery.com/cdn/shop/collections/viber_image_2026-02-19_12-42-58-503.jpg?v=1771672505',
      hebLabel: 'Featured Work', hebTitle: 'Inheritance — Raymond Loyola', href: '/works' },
  ];
  ...
  // Slide background container (sits at z-index: 1, under .hero-overlay at z-index: 2)
  const wrap = document.createElement('div');
  wrap.id = 'heroSlideshow';
  ...
  // Insert between video-bg and overlay so the overlay darkens the slides
  const overlay = hero.querySelector('.hero-overlay');
  hero.insertBefore(wrap, overlay || hero.querySelector('.hero-content') || null);

  // Progress bar (z-index: 3 — above overlay)
  const progressBar = document.createElement('div');
  progressBar.className = 'hs-progress-bar';
  hero.appendChild(progressBar);

  // Nav dots (z-index: 3 — above overlay)
  ...
  const dots      = nav.querySelectorAll('.hs-dot');
  const hebLabel  = hero.querySelector('.heb-label');
  const hebTitle  = hero.querySelector('.heb-title');
  const hebLink   = hero.querySelector('.heb-link');
  let current = 0;
  let timer   = null;
  ...
  function updateExhibitionBar(idx) {
    if (!hebLabel || !hebTitle || !hebLink) return;
    const s = SLIDES[idx];
    hebLabel.style.opacity = '0';
    hebTitle.style.opacity = '0';
    setTimeout(function() {
      hebLabel.textContent = s.hebLabel;
      hebTitle.textContent = s.hebTitle;
      hebLink.href = s.href;
      hebLabel.style.opacity = '1';
      hebTitle.style.opacity = '1';
    }, 400);
  }
  ...
  function goTo(n) {
    ...
    // Restart Ken Burns on incoming slide
    const img = slides[current].querySelector('.hs-slide-img');
    img.style.animation = 'none';
    img.offsetWidth;
    img.style.animation = '';

    updateExhibitionBar(current);
    startProgress();
    schedule();
  }
